...
export async function buildCloseTransactions(
  ...
  // The non-trapping codes (a direct read confirmed nothing on a zero-trustline account, or
  // confirmed a real detected position, per positions-gate.ts) do not refuse the build here
  // either - each carries a `code`, and plan-response.ts's own convention is that a coded
  // blocker is an acknowledged warning, not a hard stop.
