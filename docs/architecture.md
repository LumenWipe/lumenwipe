...
Aquarius is a Soroban AMM whose router deploys pools in three codes: constant-product, stableswap, and concentrated-liquidity. Each pool's LP shares live in a separate share-token contract. A share-based position (constant-product or stableswap) is withdrawn by calling the pool's own `withdraw(user, share_amount, min_amounts)`, which burns the account's shares and pays every reserve back pro rata, bounded by one floor per token in the pool's own token order; the floors are the account's share of each reserve less the slippage tolerance, read from the pool's instance at build time. Accrued AQUA rewards are read through a simulated `get_user_reward(user)` and, when there are any, claimed with `claim(user)` as their own step before the withdrawal: a merged account can never come back for them. Two conditions stop the exit with an explanation rather than lose the rewards silently - the pool's administrator has paused claiming (`kill_claim`), or the account has no authorized AQUA trustline to receive them. Concentrated-liquidity pools keep positions as tick ranges without shares and are refused by name until they have their own adapter. Pools share one code hash per type, so the registry lists the router and one representative pool per code; on testnet, where there is no position indexer, detection enumerates the router's pools and reads the account's balance in each share token.

Aquarius is also the one protocol whose execution is known to diverge from its simulation. Both `withdraw` and `claim` checkpoint the account's rewards in the pool's reward gauges, and that checkpoint writes the account's per-gauge reward entry only once the reward period has moved on since the account's last interaction. A simulation run inside the same period reads the entry and never writes it, so the footprint records it read-only, and the execution a few ledgers later traps on a write outside the footprint. The adapter's post-assembly hook (`hardenBuilt`, optional on the adapter contract) moves the account's own reward entries to the read-write footprint and adds the write capacity and fee they need; the runner re-checks the invocation, the authorization tree, and the fee cap after the hook, so it can widen what the transaction may touch but never change what it does.
