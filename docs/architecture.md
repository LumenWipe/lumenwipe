...
![Ordered nine-step demolish execution plan: normalize signers → remove data entries → claim balances → cancel offers → withdraw LP/AMM → exit DeFi → handle assets → remove trustlines → merge account](./diagrams/output/06-execution-plan.svg)

A few details that matter for correctness:

- DeFi exits come first, before any classic step, and the plan lists them in that order. Each is its own Soroban transaction (round 0 of the close), and what it withdraws lands in a trustline the classic steps then dispose of and remove. Because the exit round runs before offers are cancelled, a repay can only spend what is not held back by an open offer or the minimum reserve; a debt the spendable balance cannot cover blocks with the exact shortfall rather than building a transaction that would fail.
- Signer normalization runs next when extra signers exist, so a single key can authorize every later step. It removes each extra signer with `SetOptions` weight 0 and sets the low, medium, and high thresholds to 0/1/1. This step is a usability and efficiency choice, not a merge precondition: the protocol's subentry check excludes signers, so an account could merge with them in place. Removing them early collapses a multisig flow to one key for the remaining transactions and turns each signer's 0.5 XLM reserve into spendable balance mid-flow, where it can cover fees.
...
The fix is the protocol's fee-bump transaction (CAP-15). The user builds and signs the inner transaction in the browser exactly as in every other step, with its inner fee set to zero. The API (`POST /:network/fee-bump/sponsor`, `apps/api/src/fee-bump/`) wraps it in a fee-bump envelope whose fee source is a dedicated, lightly funded fee account and signs only the outer envelope - it does not submit; like the mediator's own co-sign step, it hands the fully-signed envelope back for the client to submit through the same `/submit` endpoint every other step already uses, rather than duplicating that retry-and-confirm logic a second time. The semantics are exact: the fee account pays the entire fee, the inner source pays nothing, and the inner transaction's signature covers its contents, so the API cannot alter an operation, an amount, or a destination without invalidating the user's signature. The fee account never touches user funds; the only thing it can spend is its own XLM, on fees.
...
- The API decodes the inner transaction and sponsors it only if every operation matches the wind-down shapes (`ChangeTrust` with limit 0, `ManageSellOffer`/`ManageBuyOffer` with amount 0, `ManageData` removals, `SetOptions` signer normalization, `ClaimClaimableBalance`, conversion payments, `AccountMerge`).
- The outer fee is capped per transaction (`MAX_FEE_BUMP_STROOPS`). Requests are rate-limited the same way every other route is, by the existing global per-API-key throttle; a dedicated per-Stellar-account or per-IP limit, and the fee account's own daily spend cap with balance alerting, are operational follow-ups rather than something this endpoint's code enforces today (`docs/threat-model.md` §6-7 tracks this precisely). Replay is structurally impossible: the inner transaction consumes the source account's sequence number, and the fee-bump wrapper carries no sequence number of its own.
...
A versioned contract registry maps each pool or vault contract's `wasmHash` to a known protocol version. An unknown `wasmHash` flags that position for manual review rather than risking an exit transaction built against the wrong interface. The same registry also vouches for the routers Section 10's conversion path is allowed to call: Soroswap's aggregator and router, and, mainnet-only and opt-in (`XBULL_CONVERSION_ENABLED`), xBull's PathPayment router - a second, best-effort quote source raced alongside Soroswap's, never in place of it.
...
Two Blend details round out the exit. BLND emissions are not reported by OctoPos, so the tool reads unclaimed emissions through the Blend SDK's emission data - per reserve token, for supplied, collateralized, and borrowed balances alike - and claims them with the pool's `claim(from, reserve_token_ids, to)` as the first step of the exit, because a merged account can never come back for them. Emissions accrue every second a balance exists, so the same fifteen-minute rule as Aquarius applies: emissions worth less than that of the position's own accrual are what the close itself produced and are left with the pool rather than claimed round after round. Receiving BLND through its Stellar Asset Contract needs a trustline; without one the exit blocks and says so. And Blend's backstop module uses a queue-for-withdrawal (Q4W) cooldown, 21 days on V1 and 17 days on V2 (the backstop token is the BLND:USDC 80/20 Comet LP share on both). The tool reads the account's deposit in the pool's backstop alongside the pool position - on testnet, detection reads the backstop's `UserBalance(pool, user)` for every registered pool - and handles it by its queue state: shares whose queue has run out are withdrawn as a step against the backstop contract (`withdraw(from, pool_address, amount)`) once the close can carry the LP token they pay out in - a Soroban token the conversion step does not handle yet (§10, #161), so until then they block with instructions; shares still cooling down, or never queued, block the exit by name with the time left. An earlier draft of this design warned and proceeded instead, but a merge with shares still in the backstop forfeits them, so a cooldown the tool cannot skip is a reason to wait, not to lose the deposit. BLND emissions accrued to the deposit itself block the same way: the tool does not claim them, because on V2 a backstop claim re-deposits them as LP shares rather than paying them out. Blend has V1 and V2 pools on mainnet, and the SDK ships both contract clients, so the tool resolves the pool version per position before building the exit.

### 9.4 Aquarius (AMM)

Aquarius is a Soroban AMM whose router deploys pools in three codes: constant-product, stableswap, and concentrated-liquidity. Each pool's LP shares live in a separate share-token contract. A share-based position (constant-product or stableswap) is withdrawn by calling the pool's own `withdraw(user, share_amount, min_amounts)`, which burns the account's shares and pays every reserve back pro rata, bounded by one floor per token in the pool's own token order; the floors are the account's share of each reserve less the slippage tolerance, read from the pool's instance at build time. Accrued AQUA rewards are read through a simulated `get_rewards_info(user)` and claimed with `claim(user)` as their own step before the withdrawal: a merged account can never come back for them. Rewards accrue for every second the account holds shares, so a claim always leaves a fresh, tiny reward behind by the time the next round reads the position; a reward worth less than fifteen minutes of the position's own emissions is taken to be what accrued during the close itself and is left with the pool rather than claimed again without end. Two conditions stop the exit with an explanation rather than lose the rewards silently - the pool's administrator has paused claiming (`kill_claim`), or the account has no authorized AQUA trustline to receive them. Concentrated-liquidity pools keep positions as tick ranges without shares and are refused by name until they have their own adapter. Pools share one code hash per type, so the registry lists the router and one representative pool per code; on testnet, where there is no position indexer, detection enumerates the router's pools and reads the account's balance in each share token.

Aquarius is also the one protocol whose execution is known to diverge from its simulation. Both `withdraw` and `claim` checkpoint the account's rewards in the pool's reward gauges, and that checkpoint writes the account's per-gauge reward entry only once the reward period has moved on since the account's last interaction. A simulation run inside the same period reads the entry and never writes it, so the footprint records it read-only, and the execution a few ledgers later traps on a write outside the footprint. The adapter's post-assembly hook (`hardenBuilt`, optional on the adapter contract) moves the account's own reward entries to the read-write footprint and adds the write capacity and fee they need. The runner requires the transaction to come back byte-for-byte identical outside its fee and Soroban resource data - source, sequence, bounds, memo, operation, arguments, authorization tree, network - and still applies the fee cap, so the hook can widen what the transaction may touch but never change what it does. It can only move keys the simulation already read; pool-level reward data is written by every simulation and needs no help.

### 9.5 Soroswap

Soroswap is a Soroban AMM. LP withdrawal calls the router's `remove_liquidity(token_a, token_b, liquidity, amount_a_min, amount_b_min, to, deadline)`, which pulls the account's LP tokens into the pair and pays both reserves back to `to`. The tool builds that call itself, against the router the contract registry vouches for, and simulates it like every other exit; the public Soroswap API (which needs an API key and would hand back bytes the tool did not build) is not used. Everything the plan needs is read from the ledger: the pair's instance holds its two tokens, both reserves, and the LP supply, and the account's shares are the pair's SEP-41 balance. The floors passed to the router are the account's share of each reserve less the slippage tolerance, a fresh quote from the very state the exit executes against. Pairs share one code hash, so the registry lists the factory, the router, and one representative pair; on testnet, where there is no position indexer, detection enumerates the factory's pairs (`TotalPairs`, `PairAddressesNIndexed(i)`) and sweeps the account's share balance across them in chunked reads. Receiving a classic asset through its Stellar Asset Contract needs a trustline; a withdrawal into an account without one is refused with the asset named rather than left to fail at the ledger.
...
Three constraints on the transfer are worth stating, because each is a consequence of the ledger rather than a product decision:

**The amount is the full trustline balance, never a user-chosen figure.** The `ChangeTrust` that removes the trustline runs immediately after and fails on a non-zero balance, so a partial transfer would strand the close midway with the account still open.
...
### 10.1 Soroban token balances

A Soroban (SEP-41) token balance is not a trustline. It lives in the token contract's own storage, keyed by holder, so no classic read shows it, and it does not stop `ACCOUNT_MERGE`: an account can be merged away with the tokens still bound to its key, recoverable only by funding that address again. That is the whole danger. A trustline with a balance makes the close fail loudly; a token balance lets it succeed silently and lose the value. So the tool treats these balances as first-class assets with their own discovery, their own decision, and their own transaction shape.

**Discovery is a union of proposers confirmed by the ledger.** There is no per-account index of token balances, and the public RPC's `getEvents` has a processing cap that makes scanning its retention window infeasible on mainnet. The API therefore gathers candidate contracts from several sources - the stellar.expert per-account listing (a third party with an undocumented endpoint: it proposes, it never decides), the payout tokens of the account's detected DeFi positions, bundled lists of Soroban-native tokens with known liquidity, recent `transfer` and `mint` events crediting the account in bounded windows, and contracts the user adds by hand (`GET /account/:address?tokens=C...,C...`) - and then confirms every candidate by simulating `balance(account)` on the RPC, reading `symbol()` and `decimals()` the same way. A Stellar asset's contract is excluded (for an account its balance is the trustline), and so is every position's own contract and share token (the position is exited by its adapter, not listed twice). Every source is time-boxed inside one overall budget; what could not be read is reported as unreadable or skipped and becomes a warning, never a silent zero. A close round re-reads state without the event scan, re-confirming the balances the analysis named and the contracts the caller's answers name.

**Every held token needs an answer, and leaving is explicit.** Each token with a balance is a required `asset_disposition` decision (`token:<contract>`) with three choices: convert to XLM (offered only when a route exists and the token reports symbol and decimals - a swap amount nobody can read is not a decision anyone can make), send the balance as the token to an account the user names (`transfer_to_account`), or acknowledge that it stays with the address (`acknowledge_residue`). The last is never pre-selected, and the API holds the plan at `needs_decisions` and refuses `/transactions` until every token has an answer. A left balance still appears as a step with nothing to sign, and on the receipt, so the record of an irreversible close carries what was left behind. There is no burn and no return-to-issuer for a Soroban-native token: it has no issuer, and a `transfer` to the contract itself would only strand the balance somewhere less recoverable.

**A transfer is one plain call, and only that.** The token round runs after the exit round (an exit can pay a token out) and before anything classic: one transaction per token, `token.transfer(account, destination, live_balance)`, simulated against the current ledger and assembled. Before it is offered, the API asserts its shape - exactly one operation, that contract, that function, those three arguments, every authorization entry under the account's own source-account credentials, none with a nested call, a fee within the cap - and refuses by name otherwise, because the token is the user's own arbitrary contract and can never be vouched for by the registry the exit runner gates on. The web anchor re-checks the same shape from the user's own inputs: the destination they typed, at least the balance they were shown, `authDepth` of zero, no account named beyond the account and its destination, no contract beyond the token. Destinations are validated live like classic transfers, minus the trustline: the account must exist, and it may be neither the account being closed nor a recognized exchange deposit address (a token transfer carries no deposit memo). The recipient needs no trustline at all.

**Conversion goes through the Soroswap API, and the API is trusted for nothing.** When conversion is enabled (`SOROBAN_CONVERSION_ENABLED` with a `SOROSWAP_API_KEY`), the plan asks the API for a quote of each held token's whole balance into XLM across the Soroswap, Phoenix, and Aquarius pools (not the classic order book: the API fills that with a classic path payment, which a Soroban-native token cannot take). A route makes the token convertible and the card shows the quoted XLM and a floor: the quote less the slippage margin, computed by LumenWipe - the API's own threshold has been observed equal to the quote itself. The convert answer carries that floor (`params.minAmountOut`), so the round that builds the swap can refuse a route that drifted under it (`quote_drifted`, 409: re-plan and accept the new rate) instead of swapping at a rate nobody agreed to. The API builds the swap for the live balance; the bytes are then decoded and held to the one shape a conversion may have before they are offered: the account's own next transaction, no memo, one `swap_exact_tokens_for_tokens` on the registry's Soroswap aggregator or router - confirmed by the contract's live code hash, not just its address - spending exactly the live balance of that token into XLM paid to the account itself, with a minimum no lower than the floor, and an authorization tree that invokes nothing but the aggregator, its adapters, the router, and the token, every `transfer` of the token the account's own and to a contract, none to a Stellar account, and no more of the token leaving than the swap spends. The browser re-checks what it can from the user's own inputs before signing: the call must be the transaction's only operation, act as the account being closed, be `swap_exact_tokens_for_tokens` on a contract from its own bundled registry, spend a token the user chose to convert, pay the proceeds to that same account, and set a minimum no lower than the figure they accepted. It does not re-check which pools the route passes through - those addresses are route-dependent and unbounded, and the browser cannot enumerate them - so that pinning is the API's, and the residual is bounded by the floor: a route through hostile pools still cannot deliver less XLM than the user agreed to. The API indexes no testnet protocol, so the conversion path is exercised by unit tests over both swap shapes and a read-only mainnet integration check of the API's own build for a real holder; a low-value mainnet close is the acceptance test.

**xBull's `strict_send` shape carries its own, narrower residual trust.** Unlike Soroswap's `path`, which names pool addresses inline, xBull's `path` argument carries indices into the router's own on-chain asset map, so the API resolves each index to a real token contract (`resolvedPath`) with a live storage read immediately before building, and re-derives it again rather than trusting what analysis produced earlier. `refs` must always be empty - a non-empty `refs` would divert part of the swap to a referral address nobody agreed to. The browser trusts the API's `resolvedPath` the same way it already trusts `nativeBalance` or `accountSigners`: not a value the user typed, but one read fresh at each stage rather than replayed from an earlier one. That catches a build that has drifted from what plan time promised; it does not catch an API that is dishonest identically at both stages. If two pending conversions would resolve to routes of the same hop count, an ambiguous match between them is refused outright rather than guessed.
...
A compromised API cannot redirect a user's funds. It builds the transactions, but the browser verifies each one against the user's own inputs before signing (`verify()`, Section 6.2), so a transaction that changed a destination, amount, or operation would fail verification and never be signed. For those checks - destination, memo, and the user's own trustline-claim choices - `verify()` takes its expected values from the user rather than the API response, so the API cannot supply its own answer. One check is sourced from the API's own trust domain rather than the user: that a `SetOptions` signer removal targets a signer that actually exists on the account is confirmed against a separate account-state read, not the transaction itself, so it catches a transaction-builder bug or a partial compromise, though a wholly compromised API could in principle keep that read and the transaction consistent with each other. Its only signing key is the shared mediator, which can co-sign only a payment whose destination and amount the user already fixed in an atomic transaction, so it can neither sign for a user's account nor redirect the forward payment. Wrong read data is caught by that same verification, on-chain simulation, and explicit confirmation of every destructive step. A DeFi exit is the one operation for which that guarantee is weaker, and it is stated here so nobody mistakes it for the same one. The client does not know a protocol's ABI, so `verify()` checks structure: the exit must be the transaction's only operation, act as the account being closed, invoke only a contract the account's own analysis showed a position in - or, for an AMM withdrawal, that protocol's router as listed in the contract registry bundled with the web (never a served copy: an entry there names a contract the user will authorize, so it reaches the list only through a code review and a deploy) - and call, on that contract, the one function that leaves the protocol (a Blend pool's `submit`, a Soroswap router's `remove_liquidity`), name no account other than the one being closed and no contract other than those positions, the token contracts of assets the account holds, and the tokens of those positions, anywhere in its arguments or in the authorization tree the signature will satisfy, carry no authorization for any other party, and pay a bounded fee. The honest limit: the position contracts and their tokens come from the API's own account read, which the user reviews but did not type. A compromised API could therefore name a contract of its choosing as a "position"; what bounds the damage is that the function it may be asked to call is fixed per protocol, that the call may act for and pay to nobody but the account, and that the runner's invariants (Section 9.9) and the simulation the user sees run before any signature. Closing the gap fully - a client-side allowlist of known pool code, or an independent read of the positions - is open work (Section 23). A passive network observer sees only TLS-protected traffic. An XSS attacker is blocked by a strict Content Security Policy with no inline scripts, no `unsafe-eval`, and no external scripts, with one intentional exception: `style-src 'self' 'unsafe-inline'` (required by stellar-wallets-kit's runtime style injection). A supply-chain attacker is constrained by lockfile-pinned dependencies, audited in CI, with no dependency permitted that needs dynamic code execution.
